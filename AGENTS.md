...
│       ├── publications/lookup/route.ts ← GET 论文元数据查询（DOI/标题/作者 → 多源候选检索；登录姓名可作为弱线索）
...
- **登录后显示**「添加论文」+「从 BibTeX 导入」按钮
  - 自动查询：输入中英文标题、DOI、DOI 链接或常见论文标识 → 优先调 Semantic Scholar，Crossref / DBLP 兜底自动补全
...
论文元数据自动补全。**Query Params**: `doi`（精确查询）或 `query`（标题、作者、标识模糊查询，返回 5 条），可附加 `author` 作为登录姓名弱线索
- 多源检索：无 DOI 时优先按自然语言标题 + 作者做候选匹配；若用户未主动提供作者，系统会把登录姓名作为弱作者线索，再结合 Semantic Scholar / Crossref 兜底；CS 方向可进一步查 DBLP
...
- ✅ 论文候选检索：`/api/publications/lookup` + AddPublicationModal（优先标题/作者自然语言匹配，未显式提供作者时自动用登录姓名作弱线索，再用 Semantic Scholar + Crossref + DBLP 多源检索）
...
**Q: 论文自动查询不工作？**
A: 现在是多源候选检索：先尽量从你输入的标题和作者里抓线索；如果你没写作者，系统还会把你的登录姓名当作弱线索，再用 Semantic Scholar + Crossref，必要时再用 DBLP 兜底。仍然找不到时，多半是该成果没有被公开学术库收录，或者标题/作者信息不够完整；这时切换到“手动录入”。查询代码在 `app/api/publications/lookup/route.ts`。
...
| 论文元数据补全 | Semantic Scholar + Crossref 多源检索 | 免费、覆盖面更广，中文标题和国内期刊可由 Crossref 兜底 |
